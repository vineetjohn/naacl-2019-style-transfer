...
The maximum length is 15 words and the vocabulary size is $\sim$9200 similar to \citet{shen2017style}.
...
The maximum length is 20 words and the vocabulary size is $\sim$58k, similar to \citet{fu2018style}.
...
For competing methods, we quote results from existing papers whenever possible. In some studies, the authors release their style-transferred sentences, and we tested them with our metrics. A caveat is that this may involve a different data spit, providing a rough (but unbiased) comparison. For others, we re-implemented the model using publicly available code. We would also wish to compare with~\newcite{hu2017toward}, but unfortunately could not find publicly available code. We look for their model performance which is compared with in subsequent work, and find that, according to the human annotation in~\newcite{shen2017style}, \newcite{hu2017toward} is comparable but slightly worse than \newcite{shen2017style}. The latter is compared with our model in terms of both automatic metrics and human evaluation.
